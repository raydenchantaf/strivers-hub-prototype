...
export default function ResourcesGrid({ resources, categories }: Props) {
  ...
  function getImageSrc(article: SanityResource, width: number, height: number) {
    const src = (language === "bm" && article.image_bm) ? article.image_bm : article.image_en;
    return src
      ? urlFor(src).width(width).height(height).fit("crop").auto("format").url()
      : "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=600&q=80";
  }

  function getCats(article: SanityResource): SanityCategory[] {
    return Array.isArray(article.category) ? article.category : [];
  }
